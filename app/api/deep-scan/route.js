...
export async function POST(request) {
  try {
    ...
    // ─── COST LOGGING (Haiku 4.5: $1/M in, $5/M out) ───
    if (data.usage) {
      const input = data.usage.input_tokens || 0;
      const output = data.usage.output_tokens || 0;
      const searchCount = (data.content || []).filter(b => b.type === "web_search_tool_result").length;
      const inputCost = (input / 1000000) * 1;
      const outputCost = (output / 1000000) * 5;
      const searchCost = searchCount * 0.01;
      const totalCost = inputCost + outputCost + searchCost;
      console.log(`[RelicID Deep Scan] Model: haiku-4.5 | Tokens: ${input} in / ${output} out | Searches: ${searchCount} | Cost: $${totalCost.toFixed(4)} (input: $${inputCost.toFixed(4)}, output: $${outputCost.toFixed(4)}, search: $${searchCost.toFixed(4)})`);
    }
